...
func (c *DefaultClient) SubscribeDeviceUplink(appID string, devID string, handler UplinkHandler) Token {
	...
	return c.subscribe(topic.String(), func(mqtt MQTT.Client, msg MQTT.Message) {
		...
		fmt.Println(dataUp)
		byteArray := msg.Payload()
		var newByte byte
		newArray := make([]byte, 0)
		for len(byteArray) > 0 {
			newByte, byteArray = byteArray[len(byteArray)-1], byteArray[:len(byteArray)-1]
			if (newByte != 91) && (newByte != 93) {
				newArray = append(newArray, newByte)
			}
		}
		err = json.Unmarshal(newArray, dataUp)
